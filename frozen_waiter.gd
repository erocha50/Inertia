...
##   ├── MeshInstance3D          (@onready body_mesh)
##   ├── AnimationPlayer         (@onready anim)
##   ├── DetectionArea           (@onready detection_area)   ← Area3D
##   │   └── CollisionShape3D    (SphereShape3D — radius set by detection_radius export)
##   └── TrayPivot               (@onready tray_pivot)       ← Node3D at enemy origin
##       └── TrayMesh            (@onready tray_mesh)        ← MeshInstance3D
##           │   Script offsets TrayMesh in -Z so root is at pivot, tip extends outward.
##           └── TrayHitbox      (@onready tray_hitbox)      ← Area3D
##               └── CollisionShape3D  (BoxShape3D — sized by tray_arm_* exports)
...
## Waiter only initiates a sweep when the player's flat speed is AT OR BELOW this value.
## Raise it to make the waiter punish slower movement too; lower it to restrict attacks
## to nearly-stationary players only.
@export var sweep_speed_threshold  : float = 4.0
@export var knockback_force       : float = 18.0
@export var detection_radius      : float = 8.0

# Tray arm — long thin stick, NOT a centred spinning box
...
# Hitbox continuous overlap tracking
...
func _tracking(delta: float) -> void:
	...
	if sweep_timer <= 0.0 and player_detected:
		# Only swing if the player is moving slowly enough to punish.
		var player_flat_spd := Vector3(player.velocity.x, 0.0, player.velocity.z).length()
		if player_flat_spd <= sweep_speed_threshold:
			_begin_windup()
		else:
			# Player is too fast — hold the timer at 0 and keep re-checking
			# each frame so the sweep fires the instant they slow down.
			sweep_timer = 0.0
...
func _begin_windup() -> void:
	...
	var player_local_x := 0.0
	if is_instance_valid(player):
		var to_player := player.global_position - global_position
		to_player.y   = 0.0
		var right      := Vector3(cos(rotation.y), 0.0, -sin(rotation.y))
		player_local_x  = to_player.dot(right)

	sweep_direction = -1.0 if player_local_x > 0.0 else 1.0
...
func _cooldown(delta: float) -> void:
	if player_detected and is_instance_valid(player):
		...
		if to_player.length() > 2.0:
			var dir := to_player.normalized()
			velocity.x = dir.x * patrol_speed * 0.5
			velocity.z = dir.z * patrol_speed * 0.5
			_face_direction(dir, delta)
		else:
			velocity.x = 0.0
			velocity.z = 0.0
...
func _tick_hitbox_sampling(delta: float) -> void:
	...
	for b in _grace_timers.keys():
		if is_instance_valid(b) and not candidates.has(b):
			candidates.append(b)
...
func _on_tray_entered(body: Node3D) -> void:
	if not _bodies_in_tray.has(body):
		_bodies_in_tray.append(body)
	if _grace_timers.has(body):
		_grace_timers.erase(body)
...
func _apply_tray_hit(body: Node3D) -> void:
	...
	# Knockback always fires — the speed gate lives in _tracking (sweep will not
	# start if the player is above sweep_speed_threshold).  Any body that enters
	# the hitbox mid-sweep is knocked back regardless of current speed.
	var away := (body.global_position - tray_pivot.global_position)
	away.y = 0.0
	if away.length_squared() < 0.001:
		away = (body.global_position - global_position)
		away.y = 0.0
	away   = away.normalized()
	away.y = 0.3
	away   = away.normalized()

	# Use take_knockback so the velocity is set directly on the player's next
	# physics frame, bypassing the _impulse * delta scaling that would otherwise
	# make the force feel negligible.
	if body.has_method("take_knockback"):
		body.take_knockback(away * knockback_force)
	else:
