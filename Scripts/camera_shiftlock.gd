...
var _lock_target:Node3D=null      # Currently locked enemy, if any
...
func _acquire_lock_target()->void:
	...
	var best:Node3D=null; var best_score:float=-1.0
	var fov_cos:=cos(deg_to_rad(lock_on_acquire_fov_deg*0.5))
	for n in get_tree().get_nodes_in_group("enemy"):
		if not (n is Node3D): continue
		if "current_health" in n and n.current_health<=0.0: continue
		var to_target:Vector3=n.global_position-global_position
		var dist:=to_target.length()
		if dist<0.01 or dist>lock_on_range: continue
		var dir:=to_target/dist
		var facing:=dir.dot(cam_fwd)
		if facing<fov_cos: continue
		var score:=facing-dist*0.01     # centred targets win; distance is a tiebreaker
		if score>best_score: best_score=score; best=n
	_lock_target=best


func _is_lock_target_valid()->bool:
	if _lock_target==null or not is_instance_valid(_lock_target): return false
	if "current_health" in _lock_target and _lock_target.current_health<=0.0: return false
	if global_position.distance_to(_lock_target.global_position)>lock_on_range*lock_on_break_range_multiplier: return false
	return true


func is_locked_on()->bool: return _lock_target!=null
func get_lock_target()->Node3D: return _lock_target
...
func _input(event:InputEvent)->void:
	...
	if event is InputEventMouseMotion:
		...
		_pitch=clampf(_pitch-event.relative.y*sensitivity_y,pitch_min,pitch_max)  # negated: mouse up = look up
		_recentre_timer=0.0
	if event is InputEventMouseButton and event.pressed:
		if   event.button_index==MOUSE_BUTTON_WHEEL_UP:   _arm_target=clampf(_arm_target-0.8,arm_min,arm_max)
			...
			if _lock_target!=null: _lock_target=null
			else: _acquire_lock_target()
...
func _process(delta:float)->void:
	...
	# While locked, orbit yaw auto-tracks the target (sun/earth — the enemy
	# is the anchor, the camera swings to keep facing it) instead of waiting
	# on mouse input or the idle recentre below.
	if _lock_target!=null and is_instance_valid(_lock_target):
		...
		if to_target.length()>0.01:
			var desired_yaw:=rad_to_deg(atan2(-to_target.x,-to_target.z))
			var yaw_diff:=wrapf(desired_yaw-_cam_yaw,-180.0,180.0)
			_cam_yaw+=yaw_diff*clampf(lock_on_orbit_speed*delta,0.0,1.0)
			_char_yaw=_cam_yaw

	_recentre_timer+=delta
	if _lock_target==null and not _freelook_active and _recentre_timer>recentre_delay:
