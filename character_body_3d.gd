...
@export var mesh_path:NodePath=^"Visual"
...
func _trigger_slam_bounce(pre_vel:Vector3, d:float)->void:
...
func _horiz(d:float)->float:
	...
	if _drifting:
		...
		velocity.x=flat.x; velocity.z=flat.z
		var str:=clampf((ang-drift_threshold)/90.0,0.0,1.0)*0.7; turn_strain.emit(str); return str
	if ang>arc_threshold and is_on_floor():
		var str:=clampf((ang-arc_threshold)/(180.0-arc_threshold),0.0,1.0); turn_strain.emit(str); return str
